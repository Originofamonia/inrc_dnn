...
def connectReservoir(self):
    ...
    connectNetworkChunks(self, fromChunks=self.exReservoirChunks, toChunks=self.exReservoirChunks,
                         mask=self.initialMasks.exex, weights=self.initialWeights.exex, prototype=exConnProto,
                         store=self.p.isWeightProbe)
    connectNetworkChunks(self, fromChunks=self.inReservoirChunks, toChunks=self.inReservoirChunks,
                         mask=self.initialMasks.inin, weights=self.initialWeights.inin, prototype=self.inConnProto)
    connectNetworkChunks(self, fromChunks=self.exReservoirChunks, toChunks=self.inReservoirChunks,
                         mask=self.initialMasks.exin, weights=self.initialWeights.exin, prototype=self.exConnProto)
    connectNetworkChunks(self, fromChunks=self.inReservoirChunks, toChunks=self.exReservoirChunks,
                         mask=self.initialMasks.inex, weights=self.initialWeights.inex, prototype=self.inConnProto)

    # Log that all cores are interconnected
    logging.info('All cores are successfully interconnected')
...
def connectOutput(self):
    ...
    connectNetworkChunks(self, fromChunks=self.exReservoirChunks, toChunks=self.outputLayerChunks, mask=self.outputMask,
                         weights=self.outputWeights, prototype=self.exConnProto)
...
def connectNetworkChunks(self, fromChunks, toChunks, mask, weights, store=False, **connectionArgs):
    ...
    for i in range(nCoresFrom):
        ...
        for j in range(nCoresTo):
            ...
            # Connect network chunks conn = fromChunks[i].connect(toChunks[j], connectionMask=ma,
            # weightExponent=self.p.weightExponent, weight=we, **connectionArgs)
